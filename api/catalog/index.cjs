...
module.exports = async (req, res) => {
  ...
  try {
    const catalogSnap = await admin.firestore()
      .collection('texa_catalog')
      .where('status', '==', 'active')
      .get();

    const tools = catalogSnap.docs.map(doc => {
      const data = doc.data();
      return {
        id: doc.id,
        name: data.name || '',
        ...
        embedVideoUrl: data.embedVideoUrl || '',
    ...
    return json(res, 500, {
      success: false,
      message: 'Gagal memuat katalog tools',
      error: error.message
    });
  }
};